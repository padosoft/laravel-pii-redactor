...
/**
 * Enforces the v1.0+ cross-pack isolation invariant for `SpainPack`.
 *
 * Every country pack MUST be self-contained:
 *
 * 1. Zero references to other country packs (`Italy\*`, `Germany\*`,
 ...
 * that makes packs interchangeable: adding a detector to `SpainPack`
 * MUST NOT silently pull a German or Italian detector class into the
 * loaded image. Shared detectors live under `src/Detectors/` and are
 * imported from there.
 *
 * The test is a no-op (passes trivially) until the SpainPack source
 * directory exists — `feature/v1.1` agents create it in parallel.
 */
final class SpainPackIsolationTest extends TestCase
{
    private const PACK_DIR = __DIR__.'/../../src/Packs/Spain';
    ...
    private const FORBIDDEN_PACK_NAMESPACES = [
        ...
        'Padosoft\\PiiRedactor\\Packs\\Germany\\',
    ...
    public function test_spain_pack_does_not_import_other_country_packs(): void
    {
        $files = $this->packFiles();

        if ($files === []) {
            $this->assertTrue(true, 'SpainPack source directory not present yet — invariant is trivially satisfied.');
        ...
        $this->assertSame([], $offenders, sprintf(
            "SpainPack imports another country pack (cross-pack isolation broken):\n%s\nShared detectors live under src/Detectors/ — import them from there, never from another Packs/<Country>/ directory.",
            implode("\n", $offenders),
        ));
    }

    public function test_spain_pack_does_not_reference_sister_package_symbols(): void
    {
        $files = $this->packFiles();
