...
- **Tenant-aware reversible vault (v1.4)** — bind a `Padosoft\PiiRedactor\Contracts\TenantResolver` (single-tenant hosts use the bundled `DefaultTenantResolver` and notice nothing) and the vault isolates per tenant: `DatabaseTokenStore` scopes every read/write by tenant (`UNIQUE(tenant_id, token)`, `clear()` wipes only the active tenant) and `TokeniseStrategy` namespaces its salt per tenant — the **same PII yields a different token per tenant** and a token can only ever be detokenised within its own tenant. Run the `add_tenant_id_to_pii_token_maps_table` migration (backfills existing rows to `default`). Fully additive: with no resolver bound, behaviour is identical to v1.3.
