...
**Official documentation:** [https://doc.laravel-pii-redactor.padosoft.com](https://doc.laravel-pii-redactor.padosoft.com)
...
- **Tenant-aware reversible vault (v1.4)** — bind a `Padosoft\PiiRedactor\Contracts\TenantResolver` (single-tenant hosts use the bundled `DefaultTenantResolver` and notice nothing) and the vault isolates per tenant on **every driver**: `DatabaseTokenStore` (rows scoped by `tenant_id`, `UNIQUE(tenant_id, token)`), `InMemoryTokenStore` (per-tenant buckets) and `CacheTokenStore` (per-tenant key namespace) scope every read/write — `clear()` wipes only the active tenant — and `TokeniseStrategy` namespaces its salt per tenant, so the **same PII yields a different token per tenant** and a token can only ever be detokenised within its own tenant. Run the `add_tenant_id_to_pii_token_maps_table` migration (backfills existing rows to `default`). Fully additive: with no resolver bound — and for the **legacy/default tenant** — behaviour is byte-for-byte identical to v1.3 (bare salt + unsegmented keys); only non-default tenants get namespaced salt + segmented keys.
